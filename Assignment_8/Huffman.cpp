 ...
 */
// 由于题目不允许将权重存储在结构体内，只能自己计算
int weightofinterval(EncodingTreeNode* interval, Map<char ,int>& weight) {
    if (interval->zero == nullptr && interval->one == nullptr) {
        return weight[interval->ch];
    }
    return weightofinterval(interval->zero, weight) + weightofinterval(interval->one, weight);
}
...
//    return nullptr;
    ...
    Map<char, int> weight;  // 存储权重
    for (auto s: str) {
        weight[s]++;
    }
    if (weight.size() == 1) {
        error("At least two differen char!");
        return nullptr;
    }
    for (auto s: weight) {
        ...
        root->zero = nullptr;
        root->one = nullptr;
        pq.enqueue(root, weight[s]);  // 将每个值按照权重加入到队列，n 个数构成森林
    }
    while (pq.size() > 1) { // 等于 1 说明哈夫曼树构造完成
        EncodingTreeNode *interval = new EncodingTreeNode;
        interval->zero = pq.dequeue();
        interval->one = pq.dequeue();
        interval->ch = ' ';
        pq.enqueue(interval, weightofinterval(interval, weight));
    }
    return pq.peek();
}
...
//    (void) tree;
//    return "";
    // 如果编码为 1，就是 one，否则 zero
    string result = "";
    EncodingTreeNode *temp;
    while (bits.size()) {
        temp = tree;
...
//    return {};
    ...
    Queue<Bit> enText = encodeText(text, tree);
    res.messageBits = enText;
    Queue<Bit> bits;
    Queue<char> leaves;
    encodeTree(tree, bits, leaves);
    res.treeBits = bits;
    res.treeLeaves = leaves;
    deleteTree(tree);
    return res;
}
