...
//    return {};
    ...
    for (GridLocation src: sources) {
        if (terrain.get(src) <= height) {
            Q.enqueue(src);
            result[src] = true;
        }
    }
    ...
    while (!Q.isEmpty()) {
        ...
        GridLocation upper(point.row - 1, point.col);
        GridLocation lower(point.row + 1, point.col);
        GridLocation left(point.row, point.col - 1);
        GridLocation right(point.row, point.col + 1);
        if (terrain.inBounds(upper) && terrain.get(upper) <= height && !result.get(upper)) {
            Q.enqueue(upper);
            result[upper] = true;
        }
        if (terrain.inBounds(lower) && terrain.get(lower) <= height && !result.get(lower)) {
            Q.enqueue(lower);
            result[lower] = true;
        }
        if (terrain.inBounds(left) && terrain.get(left) <= height && !result.get(left)) {
            Q.enqueue(left);
            result[left] = true;
        }
        if (terrain.inBounds(right) && terrain.get(right) <= height && !result.get(right)) {
            Q.enqueue(right);
            result[right] = true;
        }
    }
