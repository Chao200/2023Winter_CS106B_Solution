...
//    (void) hashFn;
    numOfslot = hashFn.numSlots();  // 赋值给 private numOfslot，便于后续处理
    myhashFn = hashFn;              // 赋值给 private myhashFn，便于后续处理
    elems = new Slot[numOfslot];    // 创建 numOfslot 个 Slot 的内存，并用 elems 指向
    sizeOfelem = 0;     // 存入的元素个数，private sizeOfelem，便于后续处理
    for (int i = 0; i < numOfslot; i++) {
        elems[i].type = SlotType::EMPTY;   // 初始化 slot 全部置为 empty
    }
}

LinearProbingHashTable::~LinearProbingHashTable() {
    /* TODO: Delete this comment, then implement this function. */
    delete[] this->elems;  // 管理内存
}

int LinearProbingHashTable::size() const {
    /* TODO: Delete this comment and the next lines, then implement this function. */
//    return -1;
    return sizeOfelem;   // 返回 slot 中存储了的数目
}

bool LinearProbingHashTable::isEmpty() const {
    /* TODO: Delete this comment and the next lines, then implement this function. */
//    return false;
    return !sizeOfelem;   // 非零为 false，表示非空
}
...
//    (void) elem;
//    return false;
    int hashcode = myhashFn(elem);   // 得到 hash 值
    if (sizeOfelem < numOfslot) {
        if (contains(elem)) {  // 如果已经存在于 slot 中，则 false
            return false;
        }
        while (elems[hashcode].type == SlotType::FILLED) { // 当有元素占用 slot，hashcode++
            hashcode++;
            hashcode %= numOfslot;
        }
        // 找到非 FILLED 的位置，存入元素，并标记为 FILLED
        ++sizeOfelem;
        elems[hashcode].type = SlotType::FILLED;
        elems[hashcode].value = elem;
        return true;
    }
    return false;
}
...
//    return false;
    ...
    int count = 0;  // 当每个 slot 都有元素存在，但是没有 elem，需要退出死循环
    while (elems[hashcode].type != SlotType::EMPTY && count++ < numOfslot) {
        if (elems[hashcode].type == SlotType::TOMBSTONE) {  // 如果是 TOMBSTONE，hashcode++
            hashcode++;
            hashcode %= numOfslot;
            continue;
        }
        if (elems[hashcode].value == elem) {  // 如果是 FILLED，并且值匹配，return true
            return true;
        }
        // 如果是 FILLED，但是值不匹配，hashcode++
        hashcode++;
        hashcode %= numOfslot;
    }
    return false;
...
//    (void) elem;
//    return false;
    if (!contains(elem)) {  // 如果不存在 elem，return false
        return false;
    }
    int hashcode = myhashFn(elem);
    while (elems[hashcode].type != SlotType::EMPTY) {  // 非空
        if (elems[hashcode].type == SlotType::FILLED && elems[hashcode].value == elem) {
            break;
        }
        hashcode++;
        hashcode %= numOfslot;
    }
    elems[hashcode].type = SlotType::TOMBSTONE;
    sizeOfelem--;
    return true;
